...
const Item = {
    normal: ({ name, message }) => [
        h('span', { className: `log-item__${name}`, key: `${Math.random()}` }, `${name}: `),
...
const LogItem = (item) => {
    ...
    return h('li', { className: `log-item ${item.type ? MessageType[item.type] : 'normal'}` }, children(item));
};
...
var setup = {"edgeLength":8,"speed":1000,"numOfTasksPerTick":1};
var players = [{"name":"John","url":"http://localhost:4001"},{"name":"Petra","url":"http://localhost:4002"},{"name":"Carmine","url":"http://localhost:4003"},{"name":"Whoopie","url":"http://localhost:4004"}];
var defaultConfig = {
	setup: setup,
	players: players
};
...
var socket = (serverUrl) => {
    ...
    return {
        ...
        onStop: action => io.on('GAME_ENDED', (scores) => action(scores)),
        onPlayerMove: action => io.on('PLAYER_MOVE_ATTEMPT', ({ name, x, y, z }) => {
            action({ name, message: `tried to move to X: ${x} Y: ${y} Z: ${z}` });
        }),
        onPlantBomb: action => io.on('PLAYER_PLACED_BOMB', ({ name, x, y, z }) => {
            action({ name, message: `placed a BOMB to X: ${x} Y: ${y} Z: ${z}` });
        }),
        onPlayerDoesNothing: action => io.on('PLAYER_DID_NOTHING', ({ name, x, y, z }) => {
            action({
                name,
                message: `decided to sit tight and do nothing at X: ${x} Y: ${y} Z: ${z}`
            });
        }),
        onPlayerLoses: action => io.on('PLAYER_LOST', ({ name, cause }) => {
            action({ name, message: `☠ ${cause}` });
        }),
        onTick: action => io.on('NEXT_TICK', (info) => {
            action(info);
        })
    };
};
...
var actions = {
    ...
    setup: {
        updateSpeed: (state, actions, speed) => (Object.assign({ speed }, state.setup))
    },
    updateGameStatus: () => (update) => {
        ...
        io.onStop((finalInfo) => {
            update((state) => {
                ...
                return Object.assign({}, state, { log, gameStatus: GameStatus.stopped });
            });
        });
    },
    // every time a socket message is received the update function will add a message to the log
    log: () => (update) => {
        io.onPlayerMove(({ name, message }) => {
            update((state) => ({
                log: [{ name, message }, ...state.log]
            }));
        });
        io.onPlantBomb(({ name, message }) => {
            update((state) => ({
                log: [{ name, message }, ...state.log]
            }));
        });
        io.onPlayerDoesNothing(({ name, message }) => {
            update((state) => ({
                log: [{ name, message }, ...state.log]
            }));
        });
        io.onPlayerLoses(({ name, message }) => {
            update((state) => ({
                log: [{ name, message, type: MessageType.special }, ...state.log]
            }));
        });
        io.onTick(({ players = [], gameInfo }) => {
            update((state) => {
...
const appActions = app({
    ...
    view: (state, actions$$1) => h('main', {}, [
        h('h1', {}, 'Battlecube'),
        h('label', {}, `Speed: ${state.setup.speed} ms`),
        Slider(state, actions$$1),
        h('button', {
            disabled: state.gameStatus === GameStatus.started,
